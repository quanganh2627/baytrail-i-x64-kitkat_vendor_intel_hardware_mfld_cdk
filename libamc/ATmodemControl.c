...
AT_STATUS at_stop()
{
	...
	RunningATcmd *pCur;
	if (!isInitialized) {
		LOGI("ATmodemControl already stopped");
		return AT_OK;
	}
	/* Stop Reader Thread*/
	LOGD("	Stopping ATmodemControl ...");
	LOGV("	Requesting to end Reader thread ...");
	pthread_mutex_lock(&at_dataMutex);  /* end reader thread correctly*/
	pthread_mutex_lock(&at_unsolicitedRespMutex);
#ifdef ANDROID 				/* Android doesn't have pthread_cancel() */
	isRequestToStop = true;	/* hack: this flag and
							 * close fdOut to escape select()*/
	close(fdOut);
#else
	...
	LOGV("	... Reader thread ended.");
	/* Release Possible Running AT Commands List Memory:
...
#ifdef ANDROID  /*pthread 'cancellation' point*/
					if ((rts == -1) && isRequestToStop)
						pthread_exit(NULL);
#endif
