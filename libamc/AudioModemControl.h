...
#define amc_unsolicitedRespMutex at_unsolicitedRespMutex
	...
	/* Acts on the full modem hw interface ( associated RX  and TX ports )
	 * HW interface should be configured prior interface activation.
	 * internal modem routing is not affected.*/
	...
	AMC_STATUS amc_route(AMC_SOURCE source, AMC_DEST dest);
	/* Several Source port routed to the same sink port induces mixing.
	 * One Source port routed to several sink port  is not always supported by
	 * the modem:
	 *  - check the feature availability through
	 *						   'amc_modem.isMultipleSinkPerSourceSupported'
	 *  - you  need to unroute the previous destination for the source first.
	 * if the related hardware interface is activated, the connection  is
	 * immediately available. Otherwise, it will be available at next interface
	 * activation.
	 * Unrouting a non-existing connection or routing an already routed one
	...
	/* use 'amc_modem.isGainControlAvailaible[port]'
	 * to know weither gain  control is supported for the given port
	 * use 'amc_modemData' data to know gain characteristics. (1 "ddB" = 0.1 dB)
	 * No out of range error is returned.
	 * gainIndex is internally bounded to stay within
	 *				 amc_modemData.gainMinDDB ... amc_modemData.gainMaxDDB.*/

	/* use 'amc_modem.isGainControlAvailaible[port]'  to know weither muting
	 * is supported for the given port*/
